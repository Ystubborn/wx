...
Page({
  ...
  loadPage: function() {
    ...
    var status = "";
    if (currentId == 1) {
      status = " and fhandlestatus='premium_status01'";
    } else if (currentId == 2) {
      status = " and fhandlestatus='premium_status05'";
    } else if (currentId == 3) {
      status = " and fhandlestatus='premium_status02'";
    }
    var filter = "";
    ...
    common.httpReq('/list/ydj_servicechange?operationno=querydata', {
      ...
      if (res.statusCode == 200 && res.data.operationResult.isSuccess) {
        ...
        for (var item of datas) {
          var status_txt = '';
          if (item.fhandlestatus == 'premium_status01') {
            status_txt = '待处理';
          } else if (item.fhandlestatus == 'premium_status05') {
            status_txt = '已驳回';
          } else if (item.fhandlestatus == 'premium_status02') {
            status_txt = '补价成功';
          }
  ...
  goOrderDetail: function(event) {
    wx.navigateTo({
      url: '/pages/order/patchOrderDetail/patchOrderDetail?id=' + event.currentTarget.dataset.id
    })
  },
