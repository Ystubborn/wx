...
Page({
  ...
  showDdxq: function () {
    this.setData({
      showDdxqFlag: !this.data.showDdxqFlag
    })
  },
  showDdxx: function () {
    this.setData({
  ...
  showFwxq: function () {
    this.setData({
      showFwxqFlag: !this.data.showFwxqFlag
    })
  },
  showAzxx: function () {
    this.setData({
  ...
  initPage: function () {
    ...
    common.httpReq('/bill/ydj_servicechange.json?operationno=initbill', {
      selectedRows: [{
      ...
      if (res.statusCode == 200 && res.data.operationResult.isSuccess) {
        var result = res.data.operationResult.srvData.uiData;
        var fserviceentry = result.fentity;
        for (var item of fserviceentry) {
          var imageArray = item.fentityimageurl.split(',');
          item.img = imageArray[0];
          item.imgs = imageArray;
        }
        that.setData({
          faccountableamount: (result.ftotalamount_chg - result.ftotalamount).toFixed(2),
          uiData: result,
          ready: true
        });
      } else {
  ...
  accoutPay: function () {
    var that = this;
    common.httpReq('/bill/ydj_servicechange?operationno=servicechangeAudit', {
      simpleData: {
        appId: app.globalData.appId
      },
      selectedRows: {
  ...
  wxpay: function () {
    ...
    var that = this;
    common.httpReq('/bill/ydj_servicechange?operationno=orderPayApplet', {
      simpleData: {
        fpaymethod: "WeChat",
  ...
  cancelOrder: function () {
    wx.navigateTo({
      url: '/pages/order/patchOrderBack/patchOrderBack?id=' + this.data.id,
    })
  }
})